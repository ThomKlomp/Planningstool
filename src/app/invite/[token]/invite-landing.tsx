...
export default function InviteLanding({
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    if (signInResult?.error) {
      setError(
        "Account is aangemaakt, maar inloggen lukte niet. Probeer in te loggen via /signin."
      );
      setLoading(false);
      return;
    }
    ...
    <main className="flex min-h-screen items-center justify-center bg-paper px-6 py-12">
      <div className="w-full max-w-sm rounded-2xl border border-line bg-white p-8 text-center">
        ...
        <p className="mt-3 text-sm text-ink/60">
          ...
          te werken bij <span className="font-medium">{companyName}</span>, voor{" "}
        ...
        {!showEmailForm ? (
          ...
          <form onSubmit={handleRegister} className="mt-6 space-y-3 text-left">
            ...
            >
              {loading ? "Bezig..." : "Account aanmaken & accepteren"}
            </button>
            <button
              type="button"
